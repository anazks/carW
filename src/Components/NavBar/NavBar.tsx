  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${
        ...
        <div className="flex justify-between items-center">
          ...
          >
            <div className="relative">
              <div className="absolute inset-0 bg-green-400 rounded-full blur-sm group-hover:blur-md transition-all duration-300 opacity-50"></div>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  <Icon className={`w-4 h-4 ${active ? 'text-green-600' : 'text-gray-500'} group-hover:text-green-600 transition-colors`} />
                  <span>{link.label}</span>
                  <span className={`absolute bottom-1 left-1/2 transform -translate-x-1/2 h-0.5 bg-green-600 transition-all duration-300 ${
      ...
      <div className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${
        ...
        <div className="px-4 pt-2 pb-4 bg-gradient-to-b from-white to-green-50/30 space-y-1">
          {navLinks.map((link) => {
            const Icon = link.icon;
            const active = isActive(link.path);
            return (
          ...
          <div className="pt-4 space-y-3 border-t border-green-100 mt-2">
            <a
              ...
              className="block mx-4 px-6 py-3 bg-gradient-to-r from-green-600 to-green-500 text-white font-semibold rounded-lg text-center hover:from-green-700 hover:to-green-600 transition-all duration-200 shadow-md relative overflow-hidden group"
            >
              <span className="relative z-10">Book Now</span>
              <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-20 transition-opacity duration-200"></div>
            </a>
